...
class DoubleHenyeyGreenstein_SPF(object):
    ...
    def __init__(self, spf_dico={"g1": 0.8, "g2": -0.2, "weight": 0.2, "scaling": 1.3e+07}):
        """
        """
        ...
        # it must contain the key "scaling"
        if "scaling" not in spf_dico.keys():
            raise TypeError("The dictionnary describing a multiple Henyey Greenstein phase function must contain the key 'scaling'")
        # the value of "weight" must be a list of floats
    ...
    def print_info(self):
        ...
        print(f"Henyey-Greenstein scaling {self.scaling=:.2f}")
    ...
    def compute_phase_function(self, phi):
        ...
        phi : float or array
            scattering angle(s) at which the scattering function must be calculated.
        """
        return self.scaling*( (self.weight * self.compute_singleHG(self.g1, phi) + (1-self.weight) * self.compute_singleHG(self.g2, phi)) /
...
#     Scattered_light_disk3.set_phase_function({'tot': True, 'spf_method':'DoubleHG', 'g1': 0.825662, 'g2': -0.225391, 'weight': 0.220596,'scaling':1.34551e+07, 
...
#     ax1.pcolormesh(disk3_tot, cmap='inferno')  # , robust = True)
#     ax1.set_xlim(0,200)
#     ax1.set_ylim(0,200)

#     ax2.set_aspect('equal', adjustable='box')
#     ax2.pcolormesh(disk3_polar, cmap='inferno')  # , robust = True)
#     ax2.set_xlim(0,200)
#     ax2.set_ylim(0,200)
